...
- **Local-first** — All identity and knowledge data stays on the user's machine. No cloud sync. **Anonymous usage statistics are off by default** — users must explicitly opt in during `engram setup`. When enabled, aggregated counts (tool names + call counts, knowledge totals, engram version) are **logged to a local file only** (`~/.engram/telemetry.log`). **No network requests are made** — the current implementation (Phase 1) is local-only logging. Never collected: identity content, prompts, file paths, IP addresses, email, or device fingerprint. Users can inspect the exact payload via `engram telemetry preview` and disable at any time with `engram telemetry off`. The optional `read_web_content` tool makes outbound HTTP to a local Reader service when explicitly invoked; core identity and knowledge tools make no network requests.
