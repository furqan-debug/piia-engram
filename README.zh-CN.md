...
| MCP 工具数 | **60** 个（默认开放 17 个 Tier-1，`ENGRAM_TOOLS=all` 开放全部 43 个）|
...
| `save_agent_context` | 自动保存 AI 会话检查点（崩溃恢复） |
| `get_recent_context` | 重启后找回丢失的会话上下文 |
| `list_agent_sessions` | 浏览各工具的历史会话记录 |

默认只加载以上 17 个核心工具。在 MCP 配置的 `env` 中设置 `ENGRAM_TOOLS=all` 可解锁全部 60 个工具。

### Tier-2 高级工具（43 个 — 知识管理、审查、导入导出）
...
60 个：17 个 Tier-1 核心工具默认加载（身份、知识、操作手册、工具图谱、项目上下文、会话恢复），43 个 Tier-2 高级工具用于知识管理、审查、导入导出和审计日志。通过 `ENGRAM_TOOLS=all` 开启全部。
