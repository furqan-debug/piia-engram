"""Round 9 regression scenarios sampled from Round 6."""
...
def _round9_copy(scenario_id: str) -> dict[str, Any]:
    source = deepcopy(ROUND6_BY_ID[scenario_id])
    source["round6_id"] = source["id"]
    source["id"] = f"R9-{source['id']}"
...
T2_SCENARIOS: list[dict[str, Any]] = [
    _round9_copy(scenario_id) for scenario_id in T2_SCENARIO_IDS
]


def validate_scenarios_v9(scenarios: list[dict[str, Any]] = T2_SCENARIOS) -> None:
    if len(scenarios) != 20:
        raise ValueError(f"Expected 20 T2 scenarios, got {len(scenarios)}")
    ids = [scenario["id"] for scenario in scenarios]
    if len(set(ids)) != len(ids):
        raise ValueError("Duplicate Round 9 scenario IDs")
