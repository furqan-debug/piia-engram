"""Run Round 7 domain-softening verification."""
...
from experiments.benchmarks.round7_domain_softening.llm_judge import LLMJudge, load_live_tools_desc
from experiments.benchmarks.round7_domain_softening.scenarios_v7 import validate_scenarios_v7
from experiments.benchmarks.round7_domain_softening.test_domain_unit import run_t1
from experiments.benchmarks.round7_domain_softening.test_regression import run_t2


ROUND7_DIR = Path(__file__).resolve().parent
...
def run_t1_group() -> dict[str, Any]:
    result = run_t1()
    _write_json(ROUND7_DIR / "results_t1.json", result)
    print(str(ROUND7_DIR / "results_t1.json"))
    return result


def run_t2_group() -> dict[str, Any]:
    validate_scenarios_v7()
    raw_path = ROUND7_DIR / "results_raw.jsonl"
    if raw_path.exists():
        raw_path.unlink()
    ...
    _write_json(ROUND7_DIR / "results_t2.json", result)
    print(str(ROUND7_DIR / "results_t2.json"))
    return result


def merge_and_report() -> dict[str, Any]:
    t1 = _read_json(ROUND7_DIR / "results_t1.json")
    t2 = _read_json(ROUND7_DIR / "results_t2.json")
    report = render_report(t1, t2)
    (ROUND7_DIR / "REPORT.md").write_text(report, encoding="utf-8")
    print(str(ROUND7_DIR / "REPORT.md"))
    return {"t1": t1, "t2": t2}
...
def render_report(t1: dict[str, Any], t2: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## 3. T2 回归结果",
            "",
            f"- Round 7：{t2['correct']}/{t2['scenario_count']}（{_pct(t2['accuracy'])}）",
