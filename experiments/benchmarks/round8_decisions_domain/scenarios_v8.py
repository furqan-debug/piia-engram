"""Round 8 regression scenarios sampled from Round 6."""
...
def _round8_copy(scenario_id: str) -> dict[str, Any]:
    source = deepcopy(ROUND6_BY_ID[scenario_id])
    source["round6_id"] = source["id"]
    source["id"] = f"R8-{source['id']}"
...
T2_SCENARIO_IDS = [
    ...
    "G1-SAVE-SNAPSHOT-01",
    "G1-DOMAINS-01",
    # G2: must include both get_decisions scenarios
    "G2-ADD-DECISION-01",
    "G2-SEARCH-01",
    "G2-RELEVANT-01",
    "G2-GET-DECISIONS-01",
...
T2_SCENARIOS: list[dict[str, Any]] = [
    _round8_copy(scenario_id) for scenario_id in T2_SCENARIO_IDS
]


def validate_scenarios_v8(scenarios: list[dict[str, Any]] = T2_SCENARIOS) -> None:
    if len(scenarios) != 20:
        raise ValueError(f"Expected 20 T2 scenarios, got {len(scenarios)}")
    ids = [scenario["id"] for scenario in scenarios]
    if len(set(ids)) != len(ids):
        raise ValueError("Duplicate Round 8 scenario IDs")

    required = {"G2-GET-DECISIONS-01", "G2-GET-DECISIONS-02"}
    present = {scenario["round6_id"] for scenario in scenarios}
    missing_required = sorted(required - present)
    if missing_required:
        raise ValueError(f"Missing required decisions scenarios: {missing_required}")
