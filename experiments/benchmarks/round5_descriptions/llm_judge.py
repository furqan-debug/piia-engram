"""DeepSeek judge for Round 5 live tool-description verification."""
...
ROUND3_ENV = ROUND5_DIR.parent / "round3" / ".env"
...
def build_tool_choice_prompt(user_input: str, tools: list[dict[str, str]]) -> str:
    return (
        ...
        "Pick exactly one best tool for the user's request. Use the tool descriptions carefully, "
        "especially the '用途' and '注意' distinctions. Return JSON only.\n\n"
        f"TOOLS:\n{_format_tools(tools)}\n\n"
        f"USER REQUEST:\n{user_input}\n\n"
        'Return exactly: {"tool": "tool_name", "reasoning": "short reason"}'
    )
...
class LLMJudge:
    def __init__(
        ...
        self.raw_log_path = raw_log_path or (ROUND5_DIR / "results_raw.jsonl")
    ...
    def judge_tool(self, scenario_id: str, user_input: str) -> dict[str, Any]:
        ...
        return _select_majority(parsed_results)
